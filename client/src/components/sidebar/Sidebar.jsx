    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-content">
            ...
            {groups.map((group) => {
              ...
              return (
                <button
                  ...
                  onClick={() => onSelectTarget({ type: 'group', id: group.id, name: group.name })}
                >
        ...
        <section className="sidebar-section">
          ...
          <div className="chat-list">
            {otherUsers.length === 0 && <div className="sidebar-empty">Других пользователей пока нет</div>}
            {otherUsers.map((user) => {
              ...
              return (
                <button
                  key={user.id}
                  className={`chat-list-item ${active ? 'active' : ''}`}
                  onClick={() => onSelectTarget({ type: 'dm', id: user.id, name: user.username })}
                >
